...
void ChatService::oneChat(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    // 查询toid是否在线 
    string value = m_redis.get_state(toid);
    if(value != "null"){
        if(value == "online"){
            cout<<"user "<<toid<<" is online in redis"<<endl;
            m_redis.publish(toid, js.dump());
        }
        else{
            cout<<"user "<<toid<<" is offline in redis"<<endl;
    ...
    // toid不在线，存储离线消息
    m_offlineMsgModel.insert(toid, js.dump());
    m_redis.set_state(toid,"offline");
}
